...
Wave 4's #249 ship. The previous escrow (still in `BusinessHub.sol` for back-compat) stored a **plaintext amount** on chain so the indexer could display it. That contradicted Blank's privacy posture — the amount is the entire point of encryption. `EncryptedEscrow.sol` ships a refactor with the amount encrypted from create through release/refund. No plaintext lookup, no leak via events.
