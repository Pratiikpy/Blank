...
- 🟡 §1.9 README polish + §4.2.8 engineering tour. **Polish pass**: 48 em-dashes cleared, Wave 4 entry added (audit iter 15 closed). Banned-word sweep clean (0 violations). **§4.2.8 added**: new "What is actually novel here" section between How-it-works and Wallets. Covers 11-file passkey-AA control flow, 3 load-bearing engineering choices, BusinessHub async-decrypt pattern, 5 privacy-aware default surfaces. Reframes the project from "testnet payments demo" to "engineering-grade passkey-AA implementation with FHE handle integration." Remaining §4 sections (4.2.1 Hero polish, 4.2.5 surface count update, 4.2.10 wave-by-wave with date corrections) deferred.
