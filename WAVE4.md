...
- 🟡 §1.4 Storefront auction phase B step 1 of 4. Added `mapping(uint256 => euint8) _winnerIdxHandle` storage variable using the canonical UUPS append-via-gap pattern (slot 7 reused from __gap, __gap reduced from 50 to 49). storage:check rebaselined for Storefront. All 19 contract baselines now reflect current state. Storefront tests still 12 passing / 2 pending (phase A revert state preserved). Steps 2-4 (FHE-tournament loop in closeAuction, revealWinner async-decrypt function, test rebuild) tracked separately.
