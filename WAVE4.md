...
- 🟡 §1.9 README polish pass (em-dash sweep + Wave 4 entry). 48 em-dashes cleared from README.md (count 48 → 0). Patterns applied: clause-separators became periods/commas, label-value bullets became colons, parenthetical asides became parens. Wave 4 entry added to the Wave summary block (was 1 wave behind reality per audit iter 15). Banned-word sweep already clean (0 violations). Full §4 spec rewrite (15 sections, 600-800 lines, 16-32 hours per §18.1.4) deferred to standalone session; this commit retires the audit's voice-rule hypocrisy on the README specifically.
